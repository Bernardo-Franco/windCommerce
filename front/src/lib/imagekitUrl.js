 ...
 */
function buildWindCommerceTextLayer({ w, h }) {
  ...
  return `l-text,i-WindCommerce,fs-${fs},co-FFFFFF,bg-0F172A90,pa-8_12,lx-N14,ly-14,lap-top_right,l-end`;
}
...
function buildTrSegment({ w, h, q = 80, f = 'auto', crop, watermark = false }) {
  ...
  return `${base}:${buildWindCommerceTextLayer({ w, h })}`;
}
...
/**
 * Same optimizations as {@link imageKitOptimizedUrl} plus WindCommerce text overlay (for share/download).
